...
import cn.tarena.weather.dao.QixiangDao;
...
private Res  res;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tv_title.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				QixiangDao  qxd=new QixiangDao();
				try {
					XResult   xResult=qxd.processCity();
					Head head=xResult.getHead();
					T.sl(getApplicationContext(), head.toString());
					T.sl(getApplicationContext(),Arrays.toString( xResult.getCitylist().toArray()));
				} catch (Exception e) {
					T.ss(getApplicationContext(), "解析失败");
					e.printStackTrace();
				}
			}
		});
	...
	private void setNet() throws UnsupportedEncodingException {
		...
		//北京  上海  广州  深圳   南京  西安  西宁  咸宁
		//拉萨   福州  郴州   武汉  长沙
		StringRequest  stringRequest=
				new StringRequest(
						"http://10.0.2.2:8080/wserver/juweather.do?city="+city,
	...
	private void setFutureWeather() {
		FutureWeatherAdapter   fwa=
				new FutureWeatherAdapter(
						res.getResult().getData().getFweatherList(),this);
	...
	private void setWeather() throws Exception {
		...
		weather_content.setText(weather.getInfo()+"|空气质量:"+pm25.getQuality());
